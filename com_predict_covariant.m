function [ mod_table ] = com_predict_covariant(tablecom,opt_sv,opt_mv)
...
for count=1:1:length(correct_position)
    sigmas(1,:)=(opt_sv(1)*((st.^(opt_sv(2)))).*(ball_velocity(count)).^(opt_sv(3))).^2;
    sigmam(1,:)= (correct_position(count).*2.^(1-(((1.4-st)-opt_mv(1))/opt_mv(2)))).^2;
    empvar(count,:)=sigmas+sigmam+sqrt(sigmas.*sigmam);
